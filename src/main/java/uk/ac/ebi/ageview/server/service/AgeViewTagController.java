 ...
 public boolean processRequest(ServiceRequest upReq, final PrintWriter out)
 {
  ...
  if(  addTags != null ||  delTags != null )
  {
   try
   {
    ...
    if(sampleId != null)
     if( !updateObjectTags(trn, sampleId, addTags, delTags, auxList, AgeViewConfigManager.SAMPLE_CLASS_NAME, out) )
      return false;

    if(groupId != null)
     if( !updateObjectTags(trn, groupId, addTags, delTags, auxList, AgeViewConfigManager.SAMPLEGROUP_CLASS_NAME, out) )
      return false;
  ...
  if( "on".equalsIgnoreCase(upReq.getParams().get(TagControlConstants.LIST_TAGS)) )
  {
   try
   {
    ...
    if(sampleId != null)
     if( !printObjectTags(rLock, sampleId, AgeViewConfigManager.SAMPLE_CLASS_NAME, out) )
      return false;
    
    if( groupId != null )
     if( !printObjectTags(rLock, groupId, AgeViewConfigManager.SAMPLEGROUP_CLASS_NAME, out) )
      return false;
